...
public class AddForceOnClick : MonoBehaviour
{
    ...
    private void AddForce()
    {
        ...
        if (Physics.Raycast(ray, out hit, _maxRayDistance) && IsHoldingLeftButtonDown && _SavedPlayerGameObject != null && hit.transform.gameObject != _SavedPlayerGameObject && _player.GetStamina() > 0f)
        {
    ...
    {
        if (_player.GetStamina() < 1f)
        {
            SetLineToZero();
        }

        // Stamina Regen.
        if (IsHoldingLeftButtonDown == false && _player.GetStamina() < 100f)
        {
