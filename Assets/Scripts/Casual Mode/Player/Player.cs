...
[RequireComponent(typeof(AddForceOnDrag))]
...
public class Player : MonoBehaviour
{
    ...
    {
        if (_godMode == false)
            if ( (collision.collider.CompareTag("Wall") || collision.collider.CompareTag("Enemy") ) && _isWin == false )
                _isDead = true;
        if (collision.collider.CompareTag("Win") && _isDead == false)
            _isWin = true;
    }
